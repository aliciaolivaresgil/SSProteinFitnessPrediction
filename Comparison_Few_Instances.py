...
def crossVal(dataset_name, general_model, n, random_state=1234, tune=False): 
...
def job(i, train_index, test_index, dataset_name, general_model, tune): 
    ...
    if 'TriTrainingRegressor' == general_model: 
        ...
        for key in models: 
            ...
            tritr = TriTrainingRegressor(base_estimator=model, y_tol_per=0.0001)
    ...
    for dataset in datasets: 
        for model in models: 
            for n in n_instances: 
                ...
                predictions, scores, tuned_params = crossVal(dataset, model, n, random_state=1234, tune=tune)
                with open(f'results/predictions_comparison_{model}_{dataset}_{str(n)}_instances.pk', 'wb') as file_predictions: 
                    pk.dump(predictions, file_predictions)
                with open(f'results/scores_comparison_{model}_{dataset}_{str(n)}_instances.pk', 'wb') as file_scores: 
                    pk.dump(scores, file_scores)
                with open(f'results/tuned_params_comparison_{model}_{dataset}_{str(n)}_instances.pk', 'wb') as file_tuned_params: 
