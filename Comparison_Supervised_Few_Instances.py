...
def crossVal(dataset_name, n, random_state=1234): 
...
if __name__=="__main__": 
    ...
    for dataset in datasets: 
        for n in n_instances: 
            ...
            predictions, scores = crossVal(dataset, n, random_state=1234)
            with open(f'results/predictions_supervised_comparison_{dataset}_{str(n)}_instances.pk', 'wb') as file_predictions: 
                pk.dump(predictions, file_predictions)
            with open(f'results/scores_supervised_comparison_{dataset}_{str(n)}_instances.pk', 'wb') as file_scores: 
