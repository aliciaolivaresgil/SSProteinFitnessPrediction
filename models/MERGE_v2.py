...
A modification is made in order to change the linear regression model for any other supervised regression model. 
...
class Merge(BaseEstimator): 
    ...
    def __init__(self, wild_type, base_regressor=Ridge(alpha=0.9)): 
        
        self.wild_type = wild_type
        self.beta_1 = None
        self.beta_2 = None
        self.base = base_regressor
    ...
    def fit(self, X_train, y_train): 
        ...
        y_base = self.base.predict(X_train)
        
        #adjust betas 
        self.beta_1, self.beta_2 = self._adjust_betas(y_train, y_dca, y_base)

    
    def predict(self, X_test): 

        y_dca = self._delta_E(X_test)
        y_base = self.base.predict(X_test)
        
        if self._spearmanr_dca >= 0: 
            return self.beta_1*y_dca + self.beta_2*y_base
        else: 
            return self.beta_1*y_dca - self.beta_2*y_base
    ...
    def _adjust_betas(self, y, y_dca, y_base): 
        loss = lambda b: -np.abs(spearmanr(y, b[0]*y_dca + b[1]*y_base)[0])
